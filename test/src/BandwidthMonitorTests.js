test("Bitrate estimation", function() {
  ...
  equal(bandwidthMonitor.getEstimatedTime(fileSize), 1000, "No history uses default bandwidth and results in a 1s estimate");
  ...
  equal(bandwidthMonitor.getEstimatedTime(fileSize), 1200, "1 entry should return the actual latency for the same filesize");
  ...
  equal(bandwidthMonitor.getEstimatedTime(fileSize), 1100, "Average latency between 2 entries");
});